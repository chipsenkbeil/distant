...
#[rstest]
fn metadata_sync_should_send_error_on_failure(ctx: &'_ DistantServerCtx) {
    let lua = lua::make().unwrap();
    let new_session = session::make_function(&lua, ctx).unwrap();
...
#[rstest]
fn metadata_sync_should_return_metadata_on_file_if_exists(ctx: &'_ DistantServerCtx) {
    let lua = lua::make().unwrap();
    let new_session = session::make_function(&lua, ctx).unwrap();
...
#[rstest]
fn metadata_sync_should_return_metadata_on_dir_if_exists(ctx: &'_ DistantServerCtx) {
    let lua = lua::make().unwrap();
    let new_session = session::make_function(&lua, ctx).unwrap();
...
#[rstest]
fn metadata_sync_should_return_metadata_on_symlink_if_exists(ctx: &'_ DistantServerCtx) {
    let lua = lua::make().unwrap();
    let new_session = session::make_function(&lua, ctx).unwrap();
...
#[rstest]
fn metadata_sync_should_include_canonicalized_path_if_flag_specified(ctx: &'_ DistantServerCtx) {
    let lua = lua::make().unwrap();
    let new_session = session::make_function(&lua, ctx).unwrap();
...
#[rstest]
fn metadata_sync_should_resolve_file_type_of_symlink_if_flag_specified(ctx: &'_ DistantServerCtx) {
    let lua = lua::make().unwrap();
    let new_session = session::make_function(&lua, ctx).unwrap();
...
#[rstest]
fn dir_read_sync_should_send_error_if_directory_does_not_exist(ctx: &'_ DistantServerCtx) {
    let lua = lua::make().unwrap();
    let new_session = session::make_function(&lua, ctx).unwrap();
...
#[rstest]
fn dir_read_sync_should_have_depth_default_to_1(ctx: &'_ DistantServerCtx) {
    let lua = lua::make().unwrap();
    let new_session = session::make_function(&lua, ctx).unwrap();
...
#[rstest]
fn dir_read_sync_should_support_depth_limits(ctx: &'_ DistantServerCtx) {
    let lua = lua::make().unwrap();
    let new_session = session::make_function(&lua, ctx).unwrap();
...
#[rstest]
fn dir_read_sync_should_support_unlimited_depth_using_zero(ctx: &'_ DistantServerCtx) {
    let lua = lua::make().unwrap();
    let new_session = session::make_function(&lua, ctx).unwrap();
...
#[rstest]
fn dir_read_sync_should_support_including_directory_in_returned_entries(ctx: &'_ DistantServerCtx) {
    let lua = lua::make().unwrap();
    let new_session = session::make_function(&lua, ctx).unwrap();
...
#[rstest]
fn dir_read_sync_should_support_returning_absolute_paths(ctx: &'_ DistantServerCtx) {
    let lua = lua::make().unwrap();
    let new_session = session::make_function(&lua, ctx).unwrap();
...
#[rstest]
fn dir_read_sync_should_support_returning_canonicalized_paths(ctx: &'_ DistantServerCtx) {
    let lua = lua::make().unwrap();
    let new_session = session::make_function(&lua, ctx).unwrap();
