...
#[rstest]
fn should_return_error_if_file_does_not_exist(ctx: &'_ DistantServerCtx) {
...
#[rstest]
fn should_send_updates_to_callback_function(ctx: &'_ DistantServerCtx) {
    let lua = lua::make().unwrap();
    let new_session = session::make_function(&lua, ctx).unwrap();
    let schedule_fn = poll::make_function(&lua).unwrap();
    ...
    let result = lua
        .load(chunk! {
            ...
            assert(watcher, "Missing file watcher")
