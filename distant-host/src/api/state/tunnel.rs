...
/// Closes a tunnel or listener, signaling graceful shutdown and removing it from
/// the map. If the target is a listener, all sub-tunnels are also closed.
///
/// Instead of aborting tasks (which can interrupt in-progress writes), this drops
/// the `write_tx` sender to signal the relay task to finish gracefully.
fn close_tunnel(id: TunnelId, tunnels: &mut HashMap<TunnelId, TunnelEntry>) -> io::Result<()> {
    match tunnels.remove(&id) {
        Some(TunnelEntry::Connection { write_tx, task, .. }) => {
            drop(write_tx);

            // Do NOT abort the task — let it finish any in-progress writes.
            // The write sub-task will see the channel close and exit,
            // then connection_task's select will also complete.
            drop(task);
            Ok(())
        }
        Some(TunnelEntry::Listener {
            ...
            for sub_id in sub_tunnel_ids {
                if let Some(TunnelEntry::Connection { write_tx, task, .. }) =
                    tunnels.remove(&sub_id)
                {
                    drop(write_tx);
                    drop(task);
                }
...
/// Writes data received on `write_rx` to the TCP stream. Uses `tokio::select!` to
/// exit when either the read side closes or the write channel is dropped (via
/// `close_tunnel`). Sends `TunnelClosed` and an `InternalRemove` message when the
/// connection ends.
async fn connection_task(
    ...
    tokio::select! {
        ...
        _ = &mut write_task => {
            // Write channel closed (close_tunnel dropped write_tx).
            // read_half will be dropped, closing the TCP read side.
        }
    }
