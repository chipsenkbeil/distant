...
/// Parsed tunnel spec from `PORT:HOST:PORT` or `PORT:PORT` format.
///
/// When only two parts are given (`PORT:PORT`), the host defaults to `"127.0.0.1"`.
///
...
pub struct TunnelSpec {
    ...
    pub bind_port: u16,
    /// The host to connect to (middle of spec, defaults to `"127.0.0.1"`).
    pub host: String,
...
impl FromStr for TunnelSpec {
    ...
    /// Parses a tunnel spec in two formats:
    /// - `PORT:HOST:PORT` — explicit host
    /// - `PORT:PORT` — host defaults to `"127.0.0.1"`
    ///
    /// Uses `rfind(':')` to handle IPv6 hosts like `[::1]` in the three-part form.
    fn from_str(spec: &str) -> Result<Self, Self::Err> {
        ...
        if let Some(last_colon) = rest.rfind(':') {
            // Three-part form: PORT:HOST:PORT
            ...
            // Two-part form: PORT:PORT (host defaults to "127.0.0.1")
            let peer_port: u16 = rest
                .parse()
                .map_err(|e| io::Error::other(format!("Invalid peer port: {e}")))?;
...
mod tests {
    ...
    mod parse {
        ...
        #[test]
        fn should_parse_two_part_spec_with_default_host() {
            let spec: TunnelSpec = "8080:5432".parse().unwrap();
            assert_eq!(spec.bind_port, 8080);
        ...
        #[test]
        fn should_parse_two_part_spec_same_port() {
            let spec: TunnelSpec = "3000:3000".parse().unwrap();
            assert_eq!(spec.bind_port, 3000);
            assert_eq!(spec.host, "127.0.0.1");
            assert_eq!(spec.peer_port, 3000);
        }
        ...
        #[test]
        fn should_reject_two_part_spec_with_invalid_peer_port() {
            let err = "8080:abc".parse::<TunnelSpec>().unwrap_err();
            assert!(
                err.to_string().contains("Invalid peer port"),
                "Expected 'Invalid peer port' in error: {}",
                err
