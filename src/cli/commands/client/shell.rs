...
/// - `\x1b[I`        — Focus In event (from `ESC[?1004h` focus tracking)
/// - `\x1b[O`        — Focus Out event (from `ESC[?1004h` focus tracking)
...
fn filter_terminal_responses(input: &[u8], out: &mut Vec<u8>) {
    ...
    while i < input.len() {
        if input[i] == 0x1b && i + 1 < input.len() && input[i + 1] == b'[' {
            ...
            // Generated by the local terminal when focus tracking is enabled
            // (via ESC[?1004h from remote stdout). Must not be forwarded.
            if i + 2 < input.len() && (input[i + 2] == b'I' || input[i + 2] == b'O') {
