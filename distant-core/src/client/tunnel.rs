...
impl RemoteTunnel {
    ...
    pub async fn open(mut channel: Channel, host: String, port: u16) -> io::Result<Self> {
        ...
        let res_task = tokio::spawn(process_tunnel_incoming(id, mailbox, reader_tx, close_tx_2));

        // Spawn the request task: reads from writer rx and close rx, sends to server
        let req_task = tokio::spawn(process_tunnel_outgoing(id, channel, writer_rx, close_rx));
...
///
/// Contains a pre-wired [`RemoteTunnelWriter`] and [`RemoteTunnelReader`] for
/// bidirectional data flow with the remote peer. Data sent through the writer
/// is forwarded to the remote side as `TunnelWrite` requests, and data arriving
/// from the remote side as `TunnelData` responses is available through the
/// reader.
#[derive(Debug)]
pub struct IncomingTunnel {
    ...
    /// Writer for sending data to the remote peer through this sub-tunnel.
    pub writer: RemoteTunnelWriter,

    /// Reader for receiving data from the remote peer through this sub-tunnel.
    pub reader: RemoteTunnelReader,
}

/// Processes outgoing data for a single sub-tunnel spawned by a reverse tunnel
/// listener.
///
/// Reads data chunks from `writer_rx` and sends them to the remote side as
/// `TunnelWrite` requests. On exit (writer dropped or channel error), sends a
/// `TunnelClose` to clean up the server-side sub-tunnel.
async fn process_sub_tunnel_outgoing(
    tunnel_id: TunnelId,
    mut channel: Channel,
    ...
    // Always send TunnelClose to clean up server-side sub-tunnel
    let _ = channel
        .fire(Request::new(protocol::Msg::Single(
            protocol::Request::TunnelClose { id: tunnel_id },
        )))
        .await;
}
...
impl RemoteTunnelListener {
    ...
    pub async fn listen(mut channel: Channel, host: String, port: u16) -> io::Result<Self> {
        ...
        // Clone channel for use inside the background task (spawning per-sub-tunnel writers)
        let channel_for_task = channel.clone();

        // Spawn a task that processes incoming connection notifications and
        // routes sub-tunnel data to the appropriate reader channels.
        let task = tokio::spawn(async move {
            let mut sub_tunnels: HashMap<TunnelId, mpsc::Sender<Vec<u8>>> = HashMap::new();
                ...
                for data in res.payload.into_vec() {
                    match data {
                        protocol::Response::TunnelIncoming {
                            ...
                            sub_tunnels.insert(tunnel_id, reader_tx);

                            tokio::spawn(process_sub_tunnel_outgoing(
                                tunnel_id,
                            ...
                            let incoming = IncomingTunnel {
                                tunnel_id,
                                peer_addr,
                                writer: RemoteTunnelWriter(writer_tx),
                                reader: RemoteTunnelReader(reader_rx),
                            };

                            if let Err(x) = tx.send(incoming).await {
                                error!(
                                    "[Listener {id}] Failed to send incoming tunnel (tunnel_id={})",
                                    x.0.tunnel_id
                                );
                                break;
                            }
                        ...
                        protocol::Response::TunnelData { id: data_id, data }
                            if sub_tunnels.contains_key(&data_id) =>
                        {
                            if let Some(reader_tx) = sub_tunnels.get(&data_id)
                                && reader_tx.send(data).await.is_err()
                            {
                                sub_tunnels.remove(&data_id);
                            }
                        }

                        protocol::Response::TunnelClosed { id: closed_id }
                            if sub_tunnels.contains_key(&closed_id) =>
                        {
                            trace!("[Listener {id}] Sub-tunnel {closed_id} closed");
                            sub_tunnels.remove(&closed_id);
                        }
...
///
/// Uses `biased` select to drain pending writes before checking the close signal,
/// preventing data loss when both branches are ready simultaneously. Always sends
/// `TunnelClose` on exit to prevent server resource leaks.
async fn process_tunnel_outgoing(
    ...
    // Main loop: forward writes, watch for close signal.
    // `biased` ensures we drain pending writes before checking the close signal.
    let close_requested = loop {
        tokio::select! {
            biased;
            data = writer_rx.recv() => {
                match data {
    ...
    // Drain any remaining buffered writes before closing
    writer_rx.close();
    while let Some(data) = writer_rx.recv().await {
        channel
            .fire(Request::new(protocol::Msg::Single(
    ...
    // Always send TunnelClose to clean up server resources
    channel
        .fire(Request::new(protocol::Msg::Single(
            protocol::Request::TunnelClose { id: tunnel_id },
        )))
